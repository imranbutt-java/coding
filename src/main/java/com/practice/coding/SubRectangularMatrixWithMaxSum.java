...
public class SubRectangularMatrixWithMaxSum {
    ...
    public Result maxSum(int input[][]){
        ...
        for(int left = 0; left < cols ; left++){
            ...
            for(int right = left; right < cols; right++){
            	//On moving right add row values of the current column in temp value
                for(int i=0; i < rows; i++){
                    temp[i] += input[i][right];
                }
                //After populating apply Kadane to get maximum sub sequence.
    ...
    public static void main(String args[]){
        int input[][] = {{ 2,  1, -3, -4,  5},
                         ...
                         { 2, -2, -1,  4, -5},
                         {-3,  3,  1,  0,  3}};
        SubRectangularMatrixWithMaxSum saw = new SubRectangularMatrixWithMaxSum();
